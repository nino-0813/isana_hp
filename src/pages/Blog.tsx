  ...
  const posts = [
    {
      ...
      excerpt: "いつもいい人と言われるけれど、恋愛に発展しない。その原因はあなたの誕生日に隠されているかもしれません。",
    ...
    <div className="pb-32">
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-20">
          {posts.map((post, i) => (
            ...
            >
              ...
              <p className="text-ink/60 leading-relaxed mb-8 line-clamp-2">
                {post.excerpt}
              </p>
              <Link
                to={`/blog/${post.id}`}
        ...
        <div className="mt-20 flex justify-center space-x-4">
          <button className="w-12 h-12 rounded-full border border-gold text-gold flex items-center justify-center hover:bg-gold hover:text-white transition-all">1</button>
          <button className="w-12 h-12 rounded-full border border-gold/20 text-ink/40 flex items-center justify-center hover:border-gold hover:text-gold transition-all">2</button>
          <button className="w-12 h-12 rounded-full border border-gold/20 text-ink/40 flex items-center justify-center hover:border-gold hover:text-gold transition-all">3</button>
        </div>
      </section>
    </div>
  );
